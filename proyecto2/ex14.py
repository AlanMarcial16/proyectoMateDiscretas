...
# NUEVA FUNCION PARA DIBUJAR CAMINO AUTOMATICO
def añadir_camino_automatico():
    """Añade varios caminos automáticos entre diferentes nodos con los pesos correspondientes."""
    caminos = [
        ("N(0,0)", "N(0,2)", 7),
        ("N(1,0)", "N(2,2)", 8),
        ("N(0,2)", "N(0,4)", 5),
    ...
    for nodo_inicial, nodo_final, peso in caminos:
        # Asignar letras a los nodos si no tienen
        letra_inicial = asignar_letra(nodo_inicial)
        letra_final = asignar_letra(nodo_final)
        
        # Resaltar los nodos
        resaltar_nodo(nodo_inicial, "green", letra_inicial)
        resaltar_nodo(nodo_final, "blue", letra_final)
        
        # Mostrar mensaje de selección de nodos
        messagebox.showinfo("Nodo Seleccionado", f"Nodo inicial seleccionado: {letra_inicial}")
        messagebox.showinfo("Nodo Seleccionado", f"Nodo final seleccionado: {letra_final}")
        
        # Añadir el camino a las conexiones
        conexiones.append((nodo_inicial, nodo_final, peso))
        
        # Dibujar el camino entre los nodos
        dibujar_camino(nodo_inicial, nodo_final, peso)
        
        # Mostrar mensaje de camino añadido
        messagebox.showinfo("Camino Automático", f"Camino automático añadido entre {nodo_inicial} y {nodo_final} con peso {peso}")
...
tk.Button(frame_inputs, text="Añadir Camino", command=añadir_camino_automatico).pack(side=tk.LEFT, padx=10)
