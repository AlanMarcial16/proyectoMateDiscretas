...
# Función para simular la selección de un nodo con un texto dado
...
# Función para simular la selección de un nodo con un texto dado
def seleccionar_nodo_texto(nodo):
    global nodo_inicial, nodo_final

    letra = asignar_letra(nodo)

    if nodo_inicial is None:
        nodo_inicial = nodo
        print(f"Nodo inicial seleccionado: {letra}")
    elif nodo_final is None:
        nodo_final = nodo
        print(f"Nodo final seleccionado: {letra}")
        solicitar_peso()


# Función para solicitar el peso de la conexión
def solicitar_peso():
...
# Función para dibujar la conexión entre dos nodos
...
# Función para dibujar el peso de la conexión
...
# Función para resaltar un nodo con el color deseado
...
# Función para generar el mapa de cuadras
...
# Función para probar la conexión entre dos nodos
def prueba_conexiones():
    # Simula la selección de nodos y dibuja el camino entre ellos
    seleccionar_nodo_texto("N(0,0)")  # Nodo inicial
    seleccionar_nodo_texto("N(1,0)")  # Nodo final

# Crear la ventana principal
root = tk.Tk()
root.title("Generador de Mapa de Calles y Cuadras")

# Frame para los controles de la interfaz
...
# Agregar un botón de prueba para ver la selección de nodos
tk.Button(frame_inputs, text="Probar Conexión", command=prueba_conexiones).pack(side=tk.LEFT, padx=10)

# Frame para el canvas donde se dibuja el mapa
