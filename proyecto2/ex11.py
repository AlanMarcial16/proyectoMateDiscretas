...
def agregar_conexiones_rapidas():
    global conexiones, nodos_letras, alfabeto, ax, canvas

    # Ventana para ingresar las conexiones
    conexiones_rapidas = simpledialog.askstring(
        "Conexiones Rápidas", 
        "Ingresa las conexiones en el formato:\nN(0,0),N(0,1),5.0\n(una conexión por línea).",
    )

    if not conexiones_rapidas:
        return  # Si no se ingresa nada, salir de la función

    for i, linea in enumerate(conexiones_rapidas.split("\n")):
        try:
            # Validar si la línea está vacía o tiene solo espacios
            ...
            partes = linea.split(",")
            if len(partes) != 3:
                raise ValueError("No tiene exactamente tres elementos separados por comas.")

            # Extraer nodos y peso
            ...
            # Validar nodos
            if not (nodo1.startswith("N(") and nodo1.endswith(")") and nodo2.startswith("N(") and nodo2.endswith(")")):
                raise ValueError(f"El formato de los nodos no es válido:\n{nodo1}, {nodo2}")

            # Extraer coordenadas
            try:
                fila1, col1 = map(int, nodo1[2:-1].split(","))  # Remover "N(" y ")" y dividir
                fila2, col2 = map(int, nodo2[2:-1].split(","))
            except ValueError:
                raise ValueError("Las coordenadas de los nodos deben ser números enteros.")

            # Validar peso
            try:
                peso = float(peso)  # Convertir peso a flotante
            except ValueError:
                raise ValueError(f"El peso '{peso}' no es un número válido.")

            # Validar que los nodos estén dentro del rango del mapa
            if not (0 <= fila1 < mapa_filas and 0 <= col1 < mapa_columnas):
                raise ValueError(f"El nodo {nodo1} está fuera del rango del mapa.")
            if not (0 <= fila2 < mapa_filas and 0 <= col2 < mapa_columnas):
                raise ValueError(f"El nodo {nodo2} está fuera del rango del mapa.")

            # Asignar letras a los nodos
            letra1 = asignar_letra(nodo1)
            letra2 = asignar_letra(nodo2)

            # Verificar si la conexión ya existe
            if (nodo1, nodo2, peso) in conexiones or (nodo2, nodo1, peso) in conexiones:
                tk.messagebox.showwarning(
                    "Conexión Duplicada",
                    f"La conexión entre {nodo1} y {nodo2} ya existe con el peso {peso}."
                )
                continue
            ...
            # Dibujar nodos resaltados
            resaltar_nodo(nodo1, "green", letra1)
            resaltar_nodo(nodo2, "blue", letra2)

        except ValueError as e:
            # Mostrar el error y la línea problemática
            tk.messagebox.showerror(
                "Error de formato",
                f"Error en la línea {i+1}:\n'{linea}'\n{str(e)}"
            )
            return

    canvas.draw()
    tk.messagebox.showinfo("Conexiones agregadas", "Las conexiones se agregaron correctamente.")
...
tk.Button(frame_inputs, text="Agregar conexiones rápidas", command=agregar_conexiones_rapidas).pack(side=tk.LEFT, padx=10)
