...
def seleccionar_nodo(event):
    ...
    # Comprobar si el nodo es una etiqueta de texto y no otro tipo de objeto
...
def dibujar_camino_resaltado(nodo1, nodo2):
    ...
    ax.plot([x1, x2], [y1, y2], color="blue", linewidth=3, zorder=2)
    canvas.draw()

# Configuración de la interfaz gráfica
root = tk.Tk()
root.title("Generador de Mapa de Caminos")
...
filas_label = tk.Label(frame_controls, text="Filas:")
filas_label.grid(row=0, column=0)
filas_entry = tk.Entry(frame_controls)
filas_entry.grid(row=0, column=1)

columnas_label = tk.Label(frame_controls, text="Columnas:")
columnas_label.grid(row=1, column=0)
columnas_entry = tk.Entry(frame_controls)
columnas_entry.grid(row=1, column=1)

# Botón para generar el mapa
generar_button = tk.Button(frame_controls, text="Generar Mapa", command=generar_mapa_cuadras)
generar_button.grid(row=2, column=0, columnspan=2, pady=10)

# Botón para reiniciar el mapa
reiniciar_button = tk.Button(frame_controls, text="Reiniciar", command=reiniciar)
reiniciar_button.grid(row=3, column=0, columnspan=2, pady=10)

# Botón para encontrar el camino más corto
camino_button = tk.Button(frame_controls, text="Encontrar Camino Más Corto", command=encontrar_camino_mas_corto)
camino_button.grid(row=4, column=0, columnspan=2, pady=10)
