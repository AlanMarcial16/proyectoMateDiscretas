...
def dibujar_camino(nodo1, nodo2, peso):
    """Dibujar un camino entre dos nodos en el gráfico."""
    global ax, canvas
    ...
    # Convertir coordenadas al sistema del gráfico
    x1, y1 = col1 + 0.5, mapa_filas - fila1 - 0.5
    x2, y2 = col2 + 0.5, mapa_filas - fila2 - 0.5

    # Dibujar línea segmentada siguiendo las calles
    if fila1 == fila2:  # Misma fila, línea horizontal
        ax.plot([x1, x2], [y1, y1], color="red", linewidth=2)
    elif col1 == col2:  # Misma columna, línea vertical
        ax.plot([x1, x1], [y1, y2], color="red", linewidth=2)
    else:  # Si están en diferentes filas y columnas, se dibujan dos líneas (L)
        ax.plot([x1, x1], [y1, y2], color="red", linewidth=2)  # Línea vertical
        ax.plot([x1, x2], [y2, y2], color="red", linewidth=2)  # Línea horizontal

    # Mostrar el peso en la mitad del camino
    peso_x = (x1 + x2) / 2
    peso_y = (y1 + y2) / 2
    ax.text(peso_x, peso_y, f"{peso:.1f}", color="red", fontsize=10, fontweight="bold")
