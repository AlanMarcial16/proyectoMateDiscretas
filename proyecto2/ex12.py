...
nodos = []  # Lista global para almacenar los nodos generados

def generar_mapa_cuadras():
    global ax, canvas, mapa_filas, mapa_columnas, nodos  # Asegúrate de incluir 'nodos' como global
...
def procesar_comandos(comandos):
    """Procesa los comandos ingresados para agregar conexiones."""
    global conexiones

    comandos = comandos.strip().split("\n")  # Separar por líneas

    for comando in comandos:
        try:
            # Validar y extraer nodos y peso
            partes = comando.split(",")
            if len(partes) != 3:
                raise ValueError(f"Formato de comando inválido: {comando}")

            nodo1, nodo2, peso = partes
            nodo1 = nodo1.strip()
            nodo2 = nodo2.strip()
            peso = float(peso.strip())  # Convertir peso a flotante

            # Validar que los nodos existen en el mapa
            if nodo1 not in nodos or nodo2 not in nodos:
                raise ValueError(f"Uno o ambos nodos no existen: {nodo1}, {nodo2}")

            # Dibujar conexión en el mapa
            conexiones.append((nodo1, nodo2, peso))  # Guardar conexión
            dibujar_camino(nodo1, nodo2, peso)      # Dibujar en el mapa
            messagebox.showinfo("Conexión Agregada", 
                                f"Conexión entre {nodo1} y {nodo2} con peso {peso} agregada exitosamente.")
        except Exception as e:
            messagebox.showerror("Error", f"Formato de comando inválido: {comando}\n{str(e)}")


def dibujar_camino2(nodo1, nodo2, peso):
    """Dibuja una conexión con su peso entre dos nodos."""
    global ax, canvas
...
def crear_interfaz_comandos():
    """Crea la interfaz para ingresar comandos de conexiones."""
    global frame_comandos

    # Evitar múltiples instancias del cuadro de comandos
    if "frame_comandos" in globals() and frame_comandos.winfo_exists():
        frame_comandos.lift()  # Traer al frente si ya existe
        return

    # Crear un nuevo Frame para los comandos
    frame_comandos = tk.Toplevel(root)  # Usar Toplevel para ventana emergente
    frame_comandos.title("Agregar Conexiones")
    frame_comandos.geometry("400x250")
    frame_comandos.configure(bg="#f5f5f5")

    # Etiqueta y entrada de texto
    etiqueta = tk.Label(frame_comandos, text="Ingrese conexiones (formato: N(i,j),N(k,l),peso):", bg="#f5f5f5")
    etiqueta.pack(side=tk.TOP, anchor="w", pady=5)

    entrada_comandos = tk.Text(frame_comandos, height=5, width=50)
    entrada_comandos.pack(side=tk.TOP, pady=5)

    # Botón para procesar los comandos
    boton_procesar = tk.Button(frame_comandos, text="Procesar Conexiones",
                               command=lambda: procesar_comandos(entrada_comandos.get("1.0", tk.END)))
    boton_procesar.pack(side=tk.TOP, pady=5)

    # Botón para cerrar el cuadro
    boton_cerrar = tk.Button(frame_comandos, text="Cerrar", command=frame_comandos.destroy)
    boton_cerrar.pack(side=tk.TOP, pady=5)
...
# Botón para agregar conexiones desde comandos
tk.Button(frame_inputs, text="Agregar Conexiones", command=crear_interfaz_comandos).pack(side=tk.LEFT, padx=10)
