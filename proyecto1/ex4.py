...
def generar_diagrama_hasse(conjunto, relaciones):
    ...
    hasse_relaciones = []
    for (a, b) in relaciones:
        # Verificar si no hay una relación intermedia entre 'a' y 'b'
    ...
    # Convertir a grafo no dirigido para eliminar flechas
    G = nx.Graph(G)

    # Generar la disposición de los nodos
    pos = nx.spring_layout(G)
    
    # Dibujar el diagrama sin flechas y sin ciclos
    nx.draw(G, pos, with_labels=True, node_size=700, node_color='lightblue', edge_color='black')
